...
Let us start with our model-building process!
...
diabetesLogTrain <- ifelse(diabetesTrain$Diabetes_binary == "1", "Yes", "No")
diabetesLogTrain <- diabetesTrain |>
  mutate(Diabetes_binary = diabetesLogTrain)

trctrl <- trainControl(method = "cv", number = 10, summaryFunction = mnLogLoss,
                       classProbs = TRUE)

logOne <- train(Diabetes_binary ~ ., data = diabetesLogTrain, method = "glm", family = "binomial",
...
logTwo <- train(Diabetes_binary ~ HighBP + HighChol + CholCheck + Stroke + HeartDiseaseorAttack +
                  HvyAlcoholConsump + DiffWalk, data = diabetesLogTrain, method = "glm", 
...
```{r}
logThree <- train(Diabetes_binary ~ HighBP*HighChol, data = diabetesLogTrain, method = "glm",
                  family = "binomial", trControl = trctrl, tuneLength = 10, metric = "logLoss")
```
